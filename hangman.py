        ...
        if get_guessed_word(secret_word,letters_guessed) == secret_word:
            ...
            for letter in secret_word:
                if letter not in unique_letters:
                    ...
                    total_score = guess * len(unique_letters)
                    answer = 'Your total score for this game is:'+str(total_score)
                    print(answer)
                    return ''
        print('__________')
        print('You have',guess,'guesses left.')
...
def match_with_gaps(my_word, other_word):
    ...
    if len(my_word) == len(other_word):
        for num in range(len(other_word)):
            if my_word[num] == '_' and other_word[num] not in my_word:
                word.append(other_word[num])
            elif my_word[num] == other_word[num]:
                word.append(other_word[num])
            else:
                return False
        return True
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guess >= 1:
        ...
        print('Available letters:',available_letters)
        user_input = input('Please guess a letter:')
        if user_input in string.ascii_letters or user_input == '*':
            if user_input in letters_guessed:
                warnings -= 1
                if warnings >= 0:
                    print('Oops! You\'ve already guessed that letter. you have',warnings, 'warnings left:',get_guessed_word(secret_word,letters_guessed))
